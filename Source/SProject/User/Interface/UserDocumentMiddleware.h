...
public:
	...
	virtual void ApplyUserDocumentChanges(const TSharedPtr<FFetchDocument> FetchDocument) = 0;

    // 인터페이스를 가진 객체들이 ApplyUserDocumentChanges를 모두 호출한 뒤 호출되는 함수
	virtual void FinalizeUserDocumentUpdate(const TSharedPtr<FFetchDocument> FetchDocument) {}
};
