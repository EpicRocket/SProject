...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MyGameplayCueManager)

//////////////////////////////////////////////////////////////////////

enum class EMyEditorLoadMode
{
...
namespace MyGameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		TEXT("My.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via MyGameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(UMyGameplayCueManager::DumpGameplayCues));

	static EMyEditorLoadMode LoadMode = EMyEditorLoadMode::LoadUpfront;
}
...
UMyGameplayCueManager* UMyGameplayCueManager::Get()
{
	return Cast<UMyGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}
...
void UMyGameplayCueManager::LoadAlwaysLoadedCues()
{
	if (ShouldDelayLoadGameplayCues())
	{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			{
				// UE_LOG(LogMy, Warning, TEXT("UMyGameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool UMyGameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (MyGameplayCueManagerCvars::LoadMode)
	{
	case EMyEditorLoadMode::LoadUpfront:
		return true;
	case EMyEditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case EMyEditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
bool UMyGameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UMyGameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UMyGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	UMyGameplayCueManager* GCM = Cast<UMyGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		// UE_LOG(LogMy, Error, TEXT("DumpGameplayCues failed. No UMyGameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	// UE_LOG(LogMy, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		// UE_LOG(LogMy, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	// UE_LOG(LogMy, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		// UE_LOG(LogMy, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				// UE_LOG(LogMy, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	// UE_LOG(LogMy, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	if (GCM->RuntimeGameplayCueObjectLibrary.CueSet)
	{
		for (const FGameplayCueNotifyData& CueData : GCM->RuntimeGameplayCueObjectLibrary.CueSet->GameplayCueData)
		{
			if (CueData.LoadedGameplayCueClass && !GCM->AlwaysLoadedCues.Contains(CueData.LoadedGameplayCueClass) && !GCM->PreloadedCues.Contains(CueData.LoadedGameplayCueClass))
			{
				...
				// UE_LOG(LogMy, Log, TEXT("  %s"), *CueData.LoadedGameplayCueClass->GetPathName());
			}
		}
	}

	// UE_LOG(LogMy, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
	// UE_LOG(LogMy, Log, TEXT("  ... %d cues in always loaded list"), GCM->AlwaysLoadedCues.Num());
	// UE_LOG(LogMy, Log, TEXT("  ... %d cues in preloaded list"), GCM->PreloadedCues.Num());
	// UE_LOG(LogMy, Log, TEXT("  ... %d cues loaded on demand"), NumMissingCuesLoaded);
	// UE_LOG(LogMy, Log, TEXT("  ... %d cues in total"), GCM->AlwaysLoadedCues.Num() + GCM->PreloadedCues.Num() + NumMissingCuesLoaded);
}

void UMyGameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (UMyGameplayCueManager* StrongThis = Get())
					{
...
void UMyGameplayCueManager::HandlePostGarbageCollect()
{
...
void UMyGameplayCueManager::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		{
			// UE_LOG(LogMy, Warning, TEXT("UMyGameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void UMyGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (MyGameplayCueManagerCvars::LoadMode)
	{
	case EMyEditorLoadMode::LoadUpfront:
...
void UMyGameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void UMyGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UMyGameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UMyGameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UMyGameplayCueManager::ShouldDelayLoadGameplayCues() const
{
...
void UMyGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
