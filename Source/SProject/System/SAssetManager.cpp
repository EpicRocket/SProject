...
static FAutoConsoleCommand CVarDumpLoadedAssets(
	...
	FConsoleCommandDelegate::CreateStatic(USAssetManager::DumpLoadedAssets)
);
...
USAssetManager& USAssetManager::Get()
{
	...
	UE_LOG(Project, Fatal, TEXT("Invalid AssetManagerClassName in DefaultEngine.ini.  It must be set to USAssetManager!"));

	// Fatal error above prevents this from being called.
	return *NewObject<USAssetManager>();
}

void USAssetManager::DumpLoadedAssets()
{
...
UObject* USAssetManager::SynchronousLoadAsset(const FSoftObjectPath& AssetPath)
{
...
bool USAssetManager::ShouldLogAssetLoads()
{
	static bool bLogAssetLoads = FParse::Param(FCommandLine::Get(), TEXT("LogAssetLoads"));
	return bLogAssetLoads;
}

void USAssetManager::AddLoadedAsset(const UObject* Asset)
{
...
{
	SCOPED_BOOT_TIMING("USAssetManager::StartInitialLoading");

	// This does all of the scanning, need to do this now even if loads are deferred
	Super::StartInitialLoading();
}

UPrimaryDataAsset* USAssetManager::LoadGameDataOfClass(TSubclassOf<UPrimaryDataAsset> DataClass,
