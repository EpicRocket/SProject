	...
	if (bSelectFriendly) {
		...
		if (abs(InitialPoint.X - CurrentPoint.X) >= 2) {
			...
			for (int32 i = 0; i < NewUnitBases.Num(); i++) {
				...
				if(NewUnitBases[i]->IsFriendly && !SUnit) // && IsActorInsideRec(IPoint, CPoint, ALocation) // && IsActorInsideRec(IPoint, CPoint, ALocation)
				{
	...
	for (int32 i = 0; i < SpeakUnits.Num(); i++)
	{
		...
		if(SpeakUnits[i]){
			...
			for (int32 u = 0; u < Units.Num(); u++) {
				...
				if(Units[u] && Units[u]->IsFriendly && !SUnit)
				{
